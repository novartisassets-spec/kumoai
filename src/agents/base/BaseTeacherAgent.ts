...
Keep it co-pilot-like and professional. Use JSON.`;
  ...
  private async handleSetup(message: RoutedMessage, schoolId: string, teacherId: string): Promise<SetupTAOutput> {
    ...
    try {
      ...
      if (parsed.internal_payload?.workload) {
          ...
          if (school?.classes_json) {
            try {
              ...
              if (Array.isArray(universeClasses) && universeClasses.length > 0) {
                ...
                // Log the expansion with school context
                logger.info({ 
                  teacherId, 
                  classCount: universeClasses.length,
                  schoolName: school.name,
                  classes: universeClasses
                }, `✅ [ALL_CLASSES] Expanded to ${universeClasses.length} classes from school universe`);
                
                // If this is a large number, the LLM should confirm with teacher
                if (universeClasses.length > 6) {
                  logger.warn({ 
                    teacherId, 
                    classCount: universeClasses.length,
                    message: 'Teacher declared ALL classes - this is a large workload'
                  }, '⚠️ Large class count detected');
                }
              } else {
            ...
            logger.warn({ schoolId }, '⚠️ No classes_json found in school record');
            finalWorkload = workload;
          }
        } else {
          finalWorkload = workload;
        ...
        for (const [classLevel, subjects] of Object.entries(finalWorkload)) {
            ...
            if (subjects === 'ALL') {
              ...
              const school: any = await new Promise((resolve) => {
                db.getDB().get(`SELECT subjects_json, name FROM schools WHERE id = ?`, [schoolId], (err, row) => resolve(row));
              });
              ...
              if (school?.subjects_json) {
                try {
                  ...
                  }, `✅ Expanded "ALL" to ${subjectList.length} subjects from school universe for ${classLevel}`);
                  ...
                  if (subjectList.length > 10) {
                    logger.warn({ 
                      teacherId, 
                      subjectCount: subjectList.length,
                      message: 'Teacher declared ALL subjects - this is a large workload'
                    }, '⚠️ Large subject count detected');
                  }
                } catch (e) {
                  logger.error({ error: e, schoolId }, '❌ Failed to parse school subjects_json');
                  subjectList = [];
                }
              } else {
                logger.warn({ schoolId }, '⚠️ No subjects_json found in school record');
                subjectList = [];
              }
      ...
      // ✅ AUTO-TRIGGER: Only trigger preview generation when teacher EXPLICITLY confirms
      // Require specific confirmation phrases - not just casual conversation
      ...
      // Only trigger on explicit confirmation phrases
      const teacherMessage = message.body?.toLowerCase() || '';
      const isExplicitConfirmation = 
        teacherMessage.includes('that\'s all') ||
        teacherMessage.includes('thats all') ||
        teacherMessage.includes('this is complete') ||
        teacherMessage.includes('list is complete') ||
        teacherMessage.includes('no more students') ||
        teacherMessage.includes('generate preview') ||
        parsed.internal_payload?.explicitly_confirmed === true;

      // Only auto-trigger if we have BOTH workload AND students AND explicit confirmation
      if (hasWorkload && hasStudents && isExplicitConfirmation && !setupData?.completed_steps?.includes('GENERATE_PREVIEW')) {
        const studentCount = setupData?.extracted_students?.length || 0;
        logger.info({ teacherId, studentCount }, '📄 AUTO-TRIGGER: Teacher explicitly confirmed list complete - generating preview PDF');
      ...
      if (canCompleteSetup) {
        ...
        if (finalStudents.length > 0) {
          ...
          for (const student of finalStudents) {
            const className = student.class_name || defaultClass;
          ...
          // Persist batch by batch for each class
          for (const [className, classStudents] of Object.entries(studentsByClass)) {
            ...
            logger.info({ teacherId, className, count: classStudents.length }, '✅ Students persisted to operational tables');
          }
        }
