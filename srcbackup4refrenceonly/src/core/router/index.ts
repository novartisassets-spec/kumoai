...
export class MessageRouter {
    ...
    static async route(msg: IncomingMessage): Promise<RoutedMessage> {
        try {
            ...
            const normalizedFrom = this.normalizePhone(msg.from);
            const normalizedParticipant = msg.participant ? this.normalizePhone(msg.participant) : undefined;
            ...
            if (msg.isGroup) {
                const senderPhone = this.normalizePhone(msg.participant || msg.from);
    ...
    private static async findSchoolByContext(msg: IncomingMessage): Promise<string | null> {
        ...
        return new Promise((resolve) => {
            db.getDB().get(
                `SELECT id FROM schools WHERE connected_whatsapp_jid = ?`,
                [msg.to],
