    ...
    static async confirmTransaction(
        ...
        return new Promise((resolve, reject) => {
            db.getDB().serialize(() => {
                ...
                db.getDB().run(auditSql, [reviewedByPhone, `transaction:${transactionId}`, JSON.stringify({ action: 'CONFIRMED', note: reviewNote })], (err) => {
                    if (err) {
                        logger.error({ error: err, transactionId }, 'Failed to log payment confirmation');
                        reject(err);
                    } else {
    ...
    static async rejectTransaction(
        ...
        return new Promise((resolve, reject) => {
            db.getDB().serialize(() => {
                ...
                db.getDB().run(auditSql, [reviewedByPhone, `transaction:${transactionId}`, JSON.stringify({ action: 'REJECTED', reason })], (err) => {
                    if (err) {
                        logger.error({ error: err, transactionId }, 'Failed to log payment rejection');
                        reject(err);
                    } else {
