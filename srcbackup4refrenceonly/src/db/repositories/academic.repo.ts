...
export class AcademicRepository {
    ...
    static async updateMark(studentId: string, subject: string, termId: string, component: string, value: number, schoolId: string, teacherId: string, studentName: string, classLevel: string): Promise<void> {
        ...
        const sql = `
            ...
            DO UPDATE SET marks_json = excluded.marks_json, total_score = excluded.total_score, updated_at = CURRENT_TIMESTAMP
        `;
        
        return new Promise((resolve, reject) => {
            db.getDB().run(sql, [uuidv4(), schoolId, studentId, studentName, teacherId, classLevel, subject, termId, JSON.stringify(marks), total], function(err) {
                if (err) {
