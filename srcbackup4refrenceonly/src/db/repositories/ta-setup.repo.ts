            ...
            const absenceCount: any = await new Promise((resolve) => {
                db.getDB().get(
                    `SELECT COUNT(*) as count FROM student_attendance_records 
                     WHERE student_id = ? AND school_id = ? AND is_present = 0 AND marked_date >= date('now', '-30 days')`,
                    [studentId, schoolId],
    ...
    static async recordManualMarkEntry(
        ...
        await new Promise<void>((resolve, reject) => {
            db.getDB().run(
                ...
                (id, school_id, student_id, student_name, teacher_id, class_level, subject, term_id, ca1, ca2, midterm, exam, manual_entry, manual_notes, confirmed_by_teacher, recorded_at)
