...
export class AgentBridgeService {
    ...
    static async relayAgentReport(
        ...
        try {
            ...
            if (saRes.reply_text) {
                ...
                if (adminPhone) {
                    await messenger.sendPush(adminPhone, saRes.reply_text);
            ...
            if (agentType === 'PA' && studentId) {
                const teacher: any = await new Promise((resolve) => {
                    const sql = `
                        SELECT u.phone, u.name, u.school_type
                        FROM users u
                ...
                if (teacher) {
                    ...
                    await messenger.sendPush(teacher.phone, taRes.reply_text);
    ...
    static async engageParentOnAbsence(
        ...
        try {
            ...
            // 4. Deliver & Record
            await messenger.sendPush(parent_phone, checkupMessage);
    ...
    static async announceToGroup(
        ...
        try {
            ...
            // 3. Deliver & Record
            await messenger.sendPush(school.whatsapp_group_jid, announcement);
