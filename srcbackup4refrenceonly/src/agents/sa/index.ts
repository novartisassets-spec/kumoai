...
const SETUP_VALIDATOR = {
    ...
    normalizePhone(phone: string | number | undefined, countryCode?: string): string | null {
        ...
        if (normalized.length === 10 && normalized.startsWith('0')) {
            const cc = countryCode || CONSTANTS.PHONE.DEFAULT_COUNTRY_CODE;
    ...
    async handle(message: RoutedMessage): Promise<any> {
        ...
        if (isNewEscalationNotification) {
            ...
            try {
                ...
                await messenger.sendPush(adminPhone, notificationText);
                ...
                if (pdfPath && fs.existsSync(pdfPath)) {
                    ...
                    await messenger.sendDocument(adminPhone, pdfPath, `📎 Attachment for Review: ${payload.reason || 'Escalation Document'}`);
                }
                    ...
                    if (!student_id || !subject || new_score === undefined) {
                        ...
                        try {
                            ...
                            if (teacherPhone) {
                                ...
                                await messenger.sendPush(teacherPhone, approvalMsg);
                            ...
                            if (broadsheet?.filePath) {
                                await messenger.sendDocument(message.from, broadsheet.filePath, `📊 Updated Broadsheet for ${class_level} (After Amendment)`);
                            }
                ...
                if (currentAction === 'DENY_MARK_AMENDMENT') {
                    ...
                    try {
                        ...
                        if (teacherPhone) {
                            ...
                            await messenger.sendPush(teacherPhone, denialMsg);
            ...
            if (output.action_required === 'CONFIRM_PAYMENT') {
                ...
                try {
                    ...
                    if (transaction) {
                        ...
                        await messenger.sendPush(transaction.payerPhone, confirmationMsg);
                ...
                try {
                    ...
                    if (transaction) {
                        ...
                        await messenger.sendPush(transaction.payerPhone, rejectionMsg);
                        ...
                        logger.info({ classLevel: workflow.class_level }, '🧬 [GOD_MODE] Proactively generating terminal reports');
                                                ...
                                                try {
                                                    ...
                                                    if (reportBatch) {
                                                        ...
                                                        await messenger.sendDocument(message.from, reportBatch.filePath, `📄 Final Terminal Reports: ${workflow.class_level}`);
            ...
            if (output.action_required === 'REQUEST_MARK_CORRECTION') {
                ...
                try {
                    ...
                    if (workflow?.teacher_phone) {
                        ...
                        await messenger.sendPush(workflow.teacher_phone, correctionMsg);
...
We'll complete your setup shortly. In the meantime, your Access Token is: *${token}*`;
            await messenger.sendPush(normalizedPhone, welcome);
...
*Term Results Token*: ${patToken} (Use this if messaging from a non-registered number).`;
            ...
            await messenger.sendPush(parent_phone, welcomeMsg);

            if (school?.whatsapp_group_link) {
                await messenger.addToGroup(parent_phone, school.whatsapp_group_link);
            }
        ...
        try {
            ...
            if (parsed.action === 'SETUP_SCHOOL' || parsed.action === 'READINESS_CONFIRMATION' || parsed.setup_status?.current_step === 'COMPLETE') {
                    ...
                    await new Promise<void>((resolve, reject) => {
                        db.getDB().run(
                            ...
                            [
                                ...
                                groupJid || null,
                                schoolType,
                                JSON.stringify(payload.universe_config?.classes_universe || []),
                            ...
                            (err) => {
                                if (err) {
                                    ...
                                    logger.info({ schoolId, groupLink: payload.school_info.whatsapp_group_link }, '✅ School info updated with universe config');
                                    resolve();
                                }
                            }
                        );
                ...
                if (payload.academic_config?.terms?.length) {
                    for (const term of payload.academic_config.terms) {
                        const termId = uuidv4();
                    ...
                    // Set active term
                    if (payload.academic_config.active_term) {
                        await new Promise(r => {
                            db.getDB().run(
                                `UPDATE schools SET active_term = ? WHERE id = ?`,
                                [payload.academic_config.active_term, schoolId],
                                () => r(null)
                            );
                        });
                    }
...
We'll complete your profile setup shortly. Your Access Token is: *${token}*`;
                        ...
                        try {
                            await messenger.sendPush(phone, welcome);
                            logger.info({ phone, name, token }, '✅ Teacher welcome sent');
                            addedTeachers.push({ teacherId, phone, name, token });
                        } catch (err) {
                            logger.error({ err, phone }, '❌ Failed to send teacher welcome');
                            addedTeachers.push({ teacherId, phone, name, token }); // Still add even if message fails
                        }
                    }
                }
        ...
        for (let i = 0; i < parents.length; i += BATCH_SIZE) {
            ...
            const batchPromises = batch.map(async (parent) => {
                try {
                    ...
                    await messenger.sendPush(parent.parentPhone, message);
